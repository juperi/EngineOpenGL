...
Camera::Camera():
		...
		yaw_(k_Yaw), pitch_(k_Pitch), fov_(k_FOV) {
			updateCameraVectors();
		}
...
Camera::Camera(const glm::vec3 position, const glm::vec3 up, const float yaw, const float pitch):
	position_(position),
	worldUp_(up),
	yaw_(yaw), pitch_(pitch), fov_(k_FOV){
		updateCameraVectors();
	}
...
void Camera::handleKeyboard(const Movement direction, const float dt) {
	float velocity = k_Speed * dt;
