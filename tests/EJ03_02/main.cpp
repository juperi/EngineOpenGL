...
void render(uint32_t VAO, const Shader& shader) {
	...
	shader.use();
	//Use sin & cos to change to multiple colors
	shader.set("uColor", 
		sin(glfwGetTime()),								//red
		cos(glfwGetTime()),								//green
		sin(2 * glfwGetTime())*cos(2 * glfwGetTime()));	//blue
...
int main(int args, char* argv[]) {
	...
	window = glfwCreateWindow(800, 600, "EJ03_02", NULL, NULL);
	if (!window) {
	...
	Shader shader("../tests/EJ03_02/vertex.vs", "../tests/EJ03_02/fragment.fs");	//Shaders path
	uint32_t VBO, EBO;
