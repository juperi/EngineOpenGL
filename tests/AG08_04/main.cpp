...
void render(uint32_t VAO, const Shader& shader_light, const Shader& shader_cube, const uint32_t tex_dif, const uint32_t tex_spec) {
	...
	shader_cube.set("light.direction", -1.0f, 0.0f, -1.0f);
	shader_cube.set("light.cutOff", glm::cos(glm::radians(20.0f)));	
	shader_cube.set("light.outerCutOff", glm::cos(glm::radians(30.0f)));
	...
	shader_cube.set("light.constant", 1.0f);
	shader_cube.set("light.linear", 0.009f);
	shader_cube.set("light.quadratic", 0.032f);
...
int main(int args, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(screen_width, screen_height, "AG08_04", NULL, NULL);
	if (!window) {
	...
	//Shaders path
	Shader shader_light("../tests/AG08_04/light.vs", "../tests/AG08_04/light.fs");
	Shader shader_cube("../tests/AG08_04/cube.vs", "../tests/AG08_04/cube.fs");
	uint32_t VBO, EBO;
	uint32_t VAO = createVertexData(&VBO, &EBO);	//Create Vertex Array Object that compiles everything

	uint32_t tex_dif = createTexture("../tests/AG08_04/albedo.png");
	uint32_t tex_spec = createTexture("../tests/AG08_04/specular.png");
