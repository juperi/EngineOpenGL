...
uint32_t createTexture(const char* path) {
	...
	// Avoided from vertex shader (1.0f - aTexCoord).
...
void render(uint32_t VAO, const Shader& shader, const uint32_t tex1) {
...
int main(int args, char* argv[]) {
	...
	window = glfwCreateWindow(800, 600, "EJ04_02", NULL, NULL);
	if (!window) {
	...
	Shader shader("../tests/EJ04_02/vertex.vs", "../tests/EJ04_02/fragment.fs");	// Shaders path
	uint32_t VBO, EBO;
	uint32_t VAO = createVertexData(&VBO, &EBO);	// Create Vertex Array Object that compiles everything

	// Create texture from images
	uint32_t tex1 = createTexture("../tests/EJ04_02/image.jpg");
