...
uint32_t createProgram1() {	//Upload and compile Shaders
	//Font code
...
uint32_t createProgram2() {	//Upload and compile Shaders
	//Font code
	const char *vertexShaderSource = "#version 330 core\n"
		"layout (location = 0) in vec3 aPos;\n"
		"void main() {\n"
...
void render(uint32_t VAO1, uint32_t VAO2, uint32_t program1, uint32_t program2) {
...
int main(int args, char* argv[]) {
	...
	window = glfwCreateWindow(800, 600, "EJ02_4", NULL, NULL);
	if (!window) {
	...
	uint32_t program1 = createProgram1();	//Upload and compile Shaders function
	uint32_t program2 = createProgram2();	//Upload and compile Shaders function
	...
	while (!glfwWindowShouldClose(window)) {	//Loop until user closes window
		...
		render(VAO1, VAO2, program1, program2);	//Paint
