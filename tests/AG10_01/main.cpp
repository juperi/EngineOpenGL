...
int main(int args, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(screen_width, screen_height, "AG07_02", NULL, NULL);
	if (!window) {
	...
	//Shaders path
	Shader shader_light("../tests/AG07_02/light.vs", "../tests/AG07_02/light.fs");
	Shader shader_cube("../tests/AG07_02/cube.vs", "../tests/AG07_02/cube.fs");
	uint32_t VBO, EBO;
	uint32_t VAO = createVertexData(&VBO, &EBO);	//Create Vertex Array Object that compiles everything

	uint32_t tex_dif = createTexture("../tests/AG07_02/albedo.png");
	uint32_t tex_spec = createTexture("../tests/AG07_02/specular.png");
