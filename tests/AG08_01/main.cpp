...
void render(uint32_t VAO, const Shader& shader_light, const Shader& shader_cube) {
	...
	model2 = glm::scale(model2, glm::vec3(10.0f, 0.5f, 10.0f));
	shader_cube.set("model", model2);
...
int main(int args, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(screen_width, screen_height, "AG08_01", NULL, NULL);
	if (!window) {
	...
	//Shaders path
	Shader shader_light("../tests/AG08_01/light.vs", "../tests/AG08_01/light.fs");
	Shader shader_cube("../tests/AG08_01/cube.vs", "../tests/AG08_01/cube.fs");
	uint32_t VBO, EBO;
