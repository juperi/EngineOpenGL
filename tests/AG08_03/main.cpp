...
int main(int args, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(screen_width, screen_height, "AG08_03", NULL, NULL);
	if (!window) {
	...
	//Shaders path
	Shader shader_light("../tests/AG08_03/light.vs", "../tests/AG08_03/light.fs");
	Shader shader_cube("../tests/AG08_03/cube.vs", "../tests/AG08_03/cube.fs");
	uint32_t VBO, EBO;
	uint32_t VAO = createVertexData(&VBO, &EBO);	//Create Vertex Array Object that compiles everything

	uint32_t tex_dif = createTexture("../tests/AG08_03/albedo.png");
	uint32_t tex_spec = createTexture("../tests/AG08_03/specular.png");
