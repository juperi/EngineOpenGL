...
void render(uint32_t VAO, const Shader& shader_light, const Shader& shader_cube) {
	...
	shader_cube.set("specularStrenght", 0.7f);
...
int main(int args, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(screen_width, screen_height, "AG06", NULL, NULL);
	if (!window) {
	...
	//Shaders path
	Shader shader_light("../tests/AG06/light.vs", "../tests/AG06/light.fs");
	Shader shader_cube("../tests/AG06/cube.vs", "../tests/AG06/cube.fs");
	uint32_t VBO, EBO;
