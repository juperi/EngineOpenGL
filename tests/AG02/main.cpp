...
uint32_t createVertexData(uint32_t *VBO, uint32_t *EBO) {	//Create VAO that compiles everything
	...
	//						1,2,3 };	//second Triangle (clockwise)
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);	//12 elements

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, *EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);	//6 elements

	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);	//Starts in vertex 0
...
int main(int args, char* argv[]) {
	...
	window = glfwCreateWindow(800, 600, "AG02", NULL, NULL);
	if (!window) {
	...
	glEnable(GL_CULL_FACE);	//Two Triangles, front and back, when one clockwise and the other counter-clockwise
	glCullFace(GL_BACK);	//Paint only front face
